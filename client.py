...
def CalPos(plaintext):
    ...
    if plaintext in local_table:
        ...
        curpos = random.randint(presum, presum + local_table[plaintext] - 1)
        #curpos = presum
        print("plaintext is",plaintext,"pos is",curpos)
        return curpos
    else:
        local_table[plaintext] = 1
        print("plaintext is",plaintext,"pos is",presum)
        return presum
...
def Search(left, right):
    ...
    rest = cur.fetchall()
    for x in rest:
        print(f"ciphtertext: {x[0]} plaintext: {Random_Decrypt(x[0])} encoding: {x[1]}")
    '''
    cur.execute(
...
if __name__ == '__main__':
    # 插入明文，同时设置了一部分重复的内容
    for ciphertext in ['apple', 'pear', 'banana', 'banana', 'banana', 'banana', 'banana', 'banana','banana','banana','banana','banana','banana']:
        print("---------insert",ciphertext,"------------")
        Insert(ciphertext)
        Search('a','z')
    # 假设我们搜索b和p之间的数据
    # Search('a', 'z')
